...
.row-card {{ background: linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01)); border-radius: 12px; padding: 14px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 12px rgba(2,6,23,0.25); border: 1px solid rgba(255,255,255,0.03); }}
...
if st.session_state.view == "cabang":
    ...
    for i in range(min(3, len(df))):
        ...
        st.markdown(f"""
        <div class='leaderboard-card' style="margin-bottom: 12px;">
            ...
            <div style='margin-top:8px; border-top:1px solid rgba(255,255,255,0.05); padding-top:8px;' class='small-muted'>
                {label_kedua}: <span style="color:white;font-weight:bold;">{fmt_num(r['total_cif'])}</span>
            </div>
...
if st.session_state.view == "pegawai":
    ...
    if dfp_all.empty:
            ...
            <div class="row-card">
                <div class="row-left">
                    ...
                    <div class="row-meta">
                        ...
                        <div class="small-muted">{label_kedua}: {cif_display}</div>
                    </div>
                </div>
                <div class="row-right">
                    <div style="margin-bottom:6px; font-size:1rem; color:var(--accent);">{end_bal}</div>
        ...
        # Navigasi halaman yang ramah jari (Mobile Friendly Buttons)
