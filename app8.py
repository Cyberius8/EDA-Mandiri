...
# ========== THEME STATE & HELPERS ==========
# ========== THEME STATE & HELPERS ==========
st.session_state.setdefault("theme_mode", "dark")  # default tetap dark
...
def get_theme():
    ...
    if mode == "light":
        return {
            ...
            "paper_bg": "#ffffff",
            "font_color": "#0f172a",
            "kpi_text": "#0f172a",               # ⬅️ teks KPI untuk light
    ...
    return {
        ...
        "paper_bg": "#0b1220",
        "font_color": "#e5e7eb",
        "kpi_text": "#ffffff",                  # ⬅️ teks KPI untuk dark
        "folium_tiles": "CartoDB dark_matter",
    }

def inject_css_theme(mode: str):
    th = get_theme()
    st.markdown(f"""
    <style>
    :root {{
      ...
      --border: {th["border"]};
      --shadow: {th["shadow"]};
      --kpi-text: {th["kpi_text"]}; /* ⬅️ baru */
    }}
    html, body, [data-testid="stAppViewContainer"] {{
      background: var(--bg);
      color: {th["font_color"]};
    }}
    .block-container {{ color: {th["font_color"]}; }}
    </style>
    """, unsafe_allow_html=True)

# inject awal sesuai state saat ini
inject_css_theme(st.session_state["theme_mode"])
...
/* (opsional) samakan header bar dengan tema */
...
/* KPI – gunakan var(--kpi-text) agar ikut tema */
/* KPI – teks putih */
.kpi {
  border-radius: 16px; padding:16px;
  color: #fff !important;    /* semua teks default putih */
...
.kpi .val,
.kpi .sub {
  color: #fff !important;    /* pastikan anak ikut putih */
  opacity: 1 !important;     /* jangan transparan */
}
.kpi .title {font-size:0.95rem; font-weight:700; margin:0}
.kpi .val   {font-size:1.9rem; font-weight:900; margin-top:4px; line-height:1.2}
.kpi .sub   {font-size:.9rem; margin-top:6px}
...
.ig-card{background:var(--panel);border:1px solid rgba(148,163,184,.15);border-radius:20px;box-shadow:0 10px 30px rgba(2,6,23,.25)}
...
.ig-tab.active{color:inherit;border-bottom:2px solid currentColor}
...
/* ===== Streamlit inputs: ikut var panel & border ===== */
[data-baseweb="input"],
[data-baseweb="select"],
...
/* Placeholder warna muted */
[data-baseweb="input"] input::placeholder {
  color: var(--muted) !important;
}

/* Popover list selectbox */
[role="listbox"]{
  background: var(--panel) !important;
  color: inherit !important;
  border: 1px solid var(--border);
...
/* Inner control agar transparan (biar warna ambil dari parent di atas) */
...
/* ====== BUTTONS (termasuk tombol area di peta) ====== */
.stButton > button{
  background: var(--panel) !important;
  color: inherit !important;
  border: 1px solid var(--border) !important;
...
/* (opsional) radio / select pill */
.stRadio [role="radiogroup"]>div,
.stSegmentedControl{ background: var(--panel) !important; border: 1px solid var(--border) !important; }

/* ====== PERBAIKAN SELECTBOX / TEXT INPUT LIGHT MODE ====== */

/* Wrapper utama select/input (hilangkan background gelap bawaan) */
...
/* Saat focus: gunakan border sesuai tema, bukan biru pekat */
[data-baseweb="select"] > div:focus-within,
[data-baseweb="input"] > div:focus-within {
  border: 1px solid var(--muted) !important;
  box-shadow: 0 0 0 1px var(--muted) !important;
}

/* Value di dalam select/input */
...
/* st.table (HTML table) */
[data-testid="stTable"] table { color: inherit !important; background: transparent !important; }
...
/* st.dataframe (Arrow grid) – paksa ambil warna tema */
.stDataFrame, .stDataFrame * { color: inherit !important; }

/* header striping halus */
.stDataFrame [role="columnheader"] {
  background: color-mix(in srgb, var(--panel) 92%, transparent) !important;
}
/* beberapa tema Streamlit memberi opacity rendah pada teks grid; naikkan */
.stDataFrame [role="gridcell"] { opacity: 1 !important; }

/* Perbaiki scrollbar dataframe biar kontrasnya pas */
.stDataFrame ::-webkit-scrollbar-thumb {
  background: color-mix(in srgb, var(--muted) 40%, transparent) !important;
  border: 1px solid var(--border) !important;
}

/* (opsional) teks bantuan/subtitle agar tidak terlalu pucat */
.small, .help, .hint, .note { color: var(--muted) !important; }

/* ===== FORM LABELS / CAPTIONS ===== */

/* Semua label selectbox, input, dll. ikut var(--font_color) */
.stSelectbox label,
.stTextInput label,
.stDateInput label,
.stMultiSelect label,
...
.stSlider label {
  color: inherit !important;
  font-weight: 600 !important;   /* opsional: biar lebih tegas */
}

/* Subtitle kecil (help, caption) */
[data-testid="stMarkdownContainer"] p,
.small, .help, .caption {
  color: var(--muted) !important;
}

/* ===== PERBAIKAN TEKS HEADER TABEL ===== */
...
/* st.dataframe header (Arrow grid) */
.stDataFrame [role="columnheader"] {
  color: inherit !important;
  opacity: 1 !important;   /* defaultnya 0.6, bikin abu */
  font-weight: 600 !important;
}

/* isi grid cell biar konsisten */
.stDataFrame [role="gridcell"] {
  color: inherit !important;
  opacity: 1 !important;
}

/* ====== STATISTIK: Tabel tahun (class .mx) & tabel card (class .cardtbl) ====== */

/* Struktur umum tabel */
table.mx, table.cardtbl {
  width: 100%;
  border-collapse: collapse;
  border-spacing: 0;
  color: inherit;                 /* ikut tema */
}

/* Header tabel — jangan abu, gunakan warna utama */
table.mx thead th,
table.cardtbl thead th {
  color: inherit !important;      /* teks jelas */
  opacity: 1 !important;
  font-weight: 700;
  text-align: center;
  padding: 10px 12px;
  background: color-mix(in srgb, var(--panel) 92%, transparent); /* head subtle */
  border-bottom: 1px solid var(--border);
}

/* Header baris kiri (PL / TC) di tabel .mx */
table.mx tbody th {
  color: inherit !important;
  opacity: 1 !important;
  font-weight: 600;
...
/* Baris key–value pada cardtbl */
table.cardtbl .col-key {
  text-align: left;
  font-weight: 600;
  width: 40%;
}

/* Badge nilai di cardtbl: kontras halus terhadap panel */
.val-badge {
...
.legend {
  ...
  color: inherit; opacity: 1;      /* jangan abu */
}
.legend .dot {
  display:inline-block; width:10px; height:10px; border-radius:999px; margin-right:6px;
}
.legend .dot.pl { background:#3b82f6; }
.legend .dot.tc { background:#f59e0b; }

/* Warna dasar pill */
.pill-b, .pill-o, .pill-muted {
  display:inline-block; min-width:56px; text-align:center;
  padding:6px 12px; border-radius:999px; font-weight:700; letter-spacing:.3px;
...
/* Sedikit radius & bayangan lembut utk tabel blok statistik */
table.mx, table.cardtbl {
  border-radius: 14px;
  overflow: hidden;
...
/* Icon next to toggle */
.mode-emoji{ font-size: 1.15rem; line-height: 1; padding-top: 6px; }

/* Compact toggle size & color */
[data-testid="stToggle"] label {
  gap: 6px !important;
}
[data-testid="stToggle"] [data-testid="stMarkdownContainer"]{ display:none !important; } /* just in case */
[data-testid="stToggle"] div[role="switch"]{
  width: 40px !important; height: 22px !important; border-radius: 999px !important;
  background: color-mix(in srgb, var(--muted) 30%, var(--panel)) !important;
  border: 1px solid var(--border) !important;
}
[data-testid="stToggle"] div[role="switch"][aria-checked="true"]{
  background: color-mix(in srgb, #0ea5e9 75%, var(--panel)) !important;
}
...
/* === Toggle with built-in moon/sun icons === */
[data-testid="stToggle"] label { gap: 6px !important; }
...
[data-testid="stToggle"] div[role="switch"]{
  ...
  background: color-mix(in srgb, var(--muted) 28%, var(--panel)) !important;
  border: 1px solid var(--border) !important;
...
/* put moon/sun icon inside the track */
...
/* active (light) track tint */
[data-testid="stToggle"] div[role="switch"][aria-checked="true"]{
  background: color-mix(in srgb, #0ea5e9 70%, var(--panel)) !important;
}

/* ===== Paksa teks tabel tidak abu-abu ===== */
...
/* ==== TEXTAREA "Catatan" (BaseWeb textarea) ==== */
[data-baseweb="textarea"],
...
/* ==== CHECKBOX & LABEL (contoh: "Ya, saya yakin") ==== */
.stCheckbox label { color: var(--font_color) !important; }
...
.stCheckbox [role="checkbox"][aria-checked="true"]{
  background: color-mix(in srgb, #0ea5e9 75%, var(--panel)) !important;
  border-color: color-mix(in srgb, #0ea5e9 75%, var(--panel)) !important;
}

/* ==== BUTTON DOWNLOAD (st.download_button) ==== */
...
.stDownloadButton > button:hover{ filter: brightness(1.03); }
.stDownloadButton > button:disabled{
  opacity: .65 !important; color: var(--muted) !important;
}

/* ==== TABEL/DAFTAR ITEM KECIL (jaga agar tak abu) ==== */
/* jika kamu pakai st.dataframe/st.table */
.stDataFrame, .stDataFrame *,
[data-testid="stTable"] *, [data-testid="stTable"] td, [data-testid="stTable"] th{
  color: var(--font_color) !important; opacity: 1 !important;
}
/* jika kamu render HTML .to_html (class dataframe) */
table.dataframe, table.dataframe td, table.dataframe th{
  color: var(--font_color) !important; opacity: 1 !important;
}

/* ==== TOMBOL AKSI KUSTOM (mis. "Hapus Item", "Unduh PDF Gerbong") ==== */
.stButton > button{
  background: var(--panel) !important;
  color: var(--font_color) !important;
  border: 1px solid var(--border) !important;
...
/* opsional: versi “primary” gelap/gradient kalau kamu pakai class btn-primary */
.btn-primary{
  display:inline-block; padding:10px 16px; border-radius:14px;
  background: linear-gradient(135deg,#0ea5e9 0%, #1d4ed8 60%, #0f172a 100%);
  border: 1px solid var(--border); color:#fff !important; text-decoration:none;
  box-shadow:0 6px 16px var(--shadow); font-weight:700;
}
.btn-primary:hover{ filter: brightness(1.06); }

/* ===== Force ALL tables/grids follow theme ===== */

/* Container frame (rounded, bg, border) */
...
/* st.dataframe (Arrow grid) – set background on every layer */
.stDataFrame, .stDataFrame * {
  color: var(--font_color) !important;
}
.stDataFrame table,
.stDataFrame thead, .stDataFrame tbody,
.stDataFrame tr, .stDataFrame th, .stDataFrame td,
.stDataFrame [role="grid"], 
.stDataFrame [role="rowgroup"],
...
/* Header striping halus agar beda dengan body */
.stDataFrame [role="columnheader"]{
  background: color-mix(in srgb, var(--panel) 92%, transparent) !important;
  color: var(--font_color) !important;
  opacity: 1 !important;
...
/* st.table (HTML) */
[data-testid="stTable"] table{ background: var(--panel) !important; }
[data-testid="stTable"] th, [data-testid="stTable"] td{
  color: var(--font_color) !important;
  background: var(--panel) !important;
  border-color: var(--border) !important;
...
/* header berbeda sedikit dari body */
.stDataFrame [role="columnheader"]{
  background: color-mix(in srgb, var(--panel) 92%, transparent) !important;
  font-weight: 600 !important;
}
...
st.markdown("""
<style>
/* ===== Arrow grid (st.dataframe) — paksa semua layer ikut tema ===== */

/* frame & latar kontainer */
...
/* tabel internal & layer-layer grid lainnya */
.stDataFrame table,
.stDataFrame thead, .stDataFrame tbody,
.stDataFrame tr, .stDataFrame th, .stDataFrame td,
.stDataFrame [role="grid"],
...
/* ===== st.table (HTML table) juga dipaksa ikut tema ===== */
[data-testid="stTable"] table{ background: var(--panel) !important; }
[data-testid="stTable"] th, [data-testid="stTable"] td{
  background: var(--panel) !important;
  color: var(--font_color) !important;
...
/* ===== textarea, checkbox, dan download button di halaman Rotasi ===== */
...
[data-baseweb="textarea"] textarea::placeholder{ color: var(--muted) !important; }
...
# Icon-only theme toggle (no label)
is_light = st.toggle("", value=(st.session_state["theme_mode"]=="light"),
                     label_visibility="collapsed")
new_mode = "light" if is_light else "dark"
if new_mode != st.session_state["theme_mode"]:
    st.session_state["theme_mode"] = new_mode
    inject_css_theme(new_mode)
    st.rerun()
...
def _apply_plot_theme(fig, th, show_legend=True):
    """Apply app theme to a Plotly figure."""
    # warna garis/grid halus
    grid = th.get("border", "rgba(148,163,184,.15)")
    tick = th.get("muted",  "#94a3b8")
    ...
    fig.update_layout(
        ...
        legend=dict(
            font=dict(color=th["font_color"]),
            orientation="h", y=-0.18 if show_legend else -0.2
        ),
        margin=dict(l=10, r=10, t=40, b=10),
    )

    # untuk chart Cartesian (bar/line), rapikan axis
    if "xaxis" in fig.layout and "yaxis" in fig.layout:
        fig.update_xaxes(
            showgrid=True, gridcolor=grid,
            zeroline=False, linecolor=grid, tickfont=dict(color=th["font_color"]),
            title_font=dict(color=th["font_color"])
        )
        fig.update_yaxes(
            showgrid=False, gridcolor=grid,
            zeroline=False, linecolor=grid, tickfont=dict(color=th["font_color"]),
            title_font=dict(color=th["font_color"])
        )
    return fig
...
def bar_grad(series, title: str = "", orientation: str = "v"):
    th = get_theme()
    ...
    # teks angka di atas bar ikut tema
    fig.update_traces(textfont=dict(color=th["font_color"], size=12))
    # rapikan layout + legend + axis
    fig.update_layout(title=title, height=360, coloraxis_showscale=False, bargap=0.25)
    fig = _apply_plot_theme(fig, th, show_legend=False)
    return fig


def donut(series, title: str = ""):
    th = get_theme()
    data = series.reset_index()
    data.columns = ["Kategori", "Jumlah"]
    ...
    # warna teks di dalam irisan: kontras dengan theme
    inner_txt = "#ffffff" if st.session_state["theme_mode"] == "dark" else "#0f172a"
    fig.update_traces(
        textinfo="value+percent",
        textposition="inside",
        textfont=dict(size=12, color=inner_txt),
        marker=dict(line=dict(color=th["paper_bg"], width=1)),
    )
    fig.update_layout(title=title, height=360)
    fig = _apply_plot_theme(fig, th, show_legend=True)
    return fig
...
def spider_chart_from_counts(counts: pd.Series, title: str = "Spider Chart — Sumber Pegawai") -> go.Figure:
    ...
    counts = (
        counts.dropna()
              .loc[counts.index.astype(str).str.strip() != ""]
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        textfont=dict(size=12, color=th["font_color"]),
        line=dict(width=2),
        marker=dict(size=6),
        name="Jumlah",
    ))
    # polar axis ikut tema: grid/tick halus
    grid = th.get("border", "rgba(148,163,184,.15)")
    tick = th.get("muted",  "#94a3b8")
    fig.update_layout(
        ...
        polar=dict(
            bgcolor=th["plot_bg"],
            radialaxis=dict(
                visible=True, range=[0, max_r * 1.1],
                gridcolor=grid, tickfont=dict(color=th["font_color"]),
            ),
            angularaxis=dict(
                direction="clockwise",
                gridcolor=grid, tickfont=dict(color=th["font_color"]),
...
def page_distribution_branch():
    ...
    fmap = folium.Map(location=[center_lat, center_lon], zoom_start=zoom_lvl, tiles=tiles)
...
def page_rotasi_mutasi():
    """Halaman Rotasi/Mutasi — CRUD Gerbong + Item Pegawai + Download PDF (portrait)."""
    import streamlit as st
    import pandas as pd
    ...
    with right:
        ...
        # === FORM TAMBAH TARGET (pilih pegawai + tujuan) ===
        st.markdown("##### 🎯 Pilih Pegawai & Tentukan Tujuan")
        ...
        # Siapkan state untuk tombol upsert (digunakan di baris aksi)
        can_upsert = bool(sel_nip and tujuan_unit and str(tujuan_jabatan).strip())
        ...
        # ===== 🔧 Baris Aksi: kiri = Hapus, kanan = Tambah/Update =====
        st.markdown("##### 🔧 Aksi Item")
        ...
        with actL:
            ...
            if j.empty:
                ...
                del_id = st.selectbox("Pilih ID item", options=id_options, key="rot_pick_del_id")
                sure = st.checkbox("Ya, saya yakin", key="rot_chk_confirm_del")
                if st.button("🗑️ Hapus Item", type="secondary", use_container_width=True,
                             disabled=(not sure), key="rot_btn_del_item"):
                    delete_item(int(del_id))
                    st.warning(f"Item #{del_id} dihapus.")
                    st.rerun()

        # -- KANAN: Tambah/Update Item --
        with actR:
            st.markdown("###### ➕ Tambah / Update Item")
            st.caption(f"Pegawai: {sel_nip} · {emp_now.get(nama_col, '—')}")
            st.caption(f"Tujuan: {tujuan_unit or '—'} — {tujuan_jabatan or '—'}")
            if st.button(label_btn, use_container_width=True, disabled=(not can_upsert), key="rot_btn_item_upsert"):
    ...
    if (
        ...
        if not merged.empty:
            ...
            st.markdown("""
                ...
                display:inline-block; padding:6px 12px; border-radius:10px;
                border:1px solid var(--border); text-decoration:none;
                color:#fff; background:linear-gradient(135deg,#0ea5e9 0%, #1d4ed8 60%, #0f172a 100%);
