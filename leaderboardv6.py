...
if st.session_state.view == "cabang":
    ...
    for _, r in df.iterrows():
        kode_cb = r['kode_cabang']
        unit = r['unit']
        area = r['area']
        # HTML row with left meta and right detail link
        row_html = f"""
        <div class="row-entry" role="listitem" aria-label="Cabang {unit}">
        <div class="row-left">
            <div style="font-weight:700; margin-right:8px; min-width:40px;">{kode_cb}</div>
            <div class="row-meta">
            <div class="unit">{unit}</div>
            <div class="subcode">{area}</div>
            </div>
        </div>
        <div>
